...
def setupLogging(name):
    ...
    fileHandler = logging.handlers.RotatingFileHandler(logFile, maxBytes=10485760, backupCount=2)
...
def saveObject(ol, ObjectFile):
    try:
        logger.debug("Loaded : %s" % ObjectFile)
        with open(ObjectFile, "wb") as cf:
            pickle.dump(ol, cf)

    except IOError, msg:
        logger.error("%s - %s " % (msg, str(sys.exc_info()[0])))


def loadObject(ObjectFile):

    def createNone():
        d = dict()
        d[ObjectFile] = list([0, ObjectFile])
        return d

    if not os.path.exists(ObjectFile):
        logger.error("%s : Does Not Exist!" % ObjectFile)

    try:
        with open(ObjectFile, "rb")as cf:
            pl = pickle.load(cf)
            logger.debug("Loaded : %s" % ObjectFile)
            return pl
