...
if __name__ == "__main__":
    ...
    berlin = xyz(52.518611, 13.408056)
    munich = xyz(48.137222, 11.575556)

    # Get distance between two points on a sphere
    meters = distance(berlin, munich) / 1000.0
    print("%3.3f KM" % meters)

    ang = angle(berlin, munich)
    ang_s = direction_name(ang)
    print("%3.3f %s : Angle" % (ang, ang_s))

    # Determine travelling direction for a compass
    print("%3.3f Compass" % great_circle_angle(munich, berlin, magnetic_northpole))

    true_north = great_circle_angle(munich, berlin, geographic_northpole)
    print("%3.3f Heading" % true_north)
