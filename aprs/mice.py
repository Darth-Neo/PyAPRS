...
def parse_mice(dstcall, body):
    ...
    # verify mic-e format
    if len(dstcall) != 6:
        raise Exception("dstcall has to be 6 characters")
    if len(body) < 8:
        raise Exception("packet data field is too short")
    if not re.match(r"^[0-9A-Z]{3}[0-9L-Z]{3}$", dstcall):
        raise Exception("invalid dstcall")
    if not re.match(r"^[&-\x7f][&-a][\x1c-\x7f]{2}[\x1c-\x7d]"
                    r"[\x1c-\x7f][\x21-\x7e][\/\\0-9A-Z]", body):
        raise Exception("invalid data format")

    # get symbol table and symbol
    pd = {"symbol": body[6],"symbol_table": body[7]}
    parsed.update(pd)
    ...
    if not match:
        raise Exception("invalid latitude ambiguity")

    posambiguity = len(match[0])
    pd =  {"posambiguity": "{}".format(posambiguity)}
    parsed.update(pd)
    ...
    if mbits.find("2") > -1:
        pd = {"mtype": MTYPE_TABLE_CUSTOM[mbits.replace("2", "1")]}
        parsed.update(pd)
    else:
        pd = {"mtype": MTYPE_TABLE_STD[mbits]}
        parsed.update(pd)
    ...
    longitude += -80 if 180 >= longitude <= 189 else 0
    #longitude += -190 if longitude >= 190 and longitude <= 199 else 0
    longitude += -190 if 190 <= longitude <= 199 else 0
    ...
    if posambiguity is 4:
        ...
        raise Exception("Unsupported position ambiguity: %d" % posambiguity)
