import re
from base91 import *

from Logger import *
logger = setupLogging(__name__)
logger.setLevel(INFO)

__all__ = [
        "parse_comment_telemetry",
        "parse_telemetry_config",
        ]


def parse_comment_telemetry(text):
    """
    Looks for base91 telemetry found in comment field
    Returns [remaining_text, telemetry]
    """
    parsed = dict()
    match = re.findall(r"^(.*?)\|([!-{]{4,14})\|(.*)$", text)

    if match and len(match[0][1]) % 2 == 0:
        text, telemetry, post = match[0]
        text += post

        temp = [0] * 7
        for i in range(7):
            temp[i] = to_decimal(telemetry[i*2:i*2+2])

        pd = {"telemetry": {"seq": temp[0],"vals": temp[1:6]}}
        parsed.update(pd)
        if temp[6] != "":
            pd = {"bits": "{0:08b}".format(temp[6] & 0xFF)[::-1]}
            parsed["telemetry"].update(pd)

    rl = (text, parsed)
    return rl


def parse_telemetry_config(body):
    parsed = {}

    match = re.findall(r"^(PARM|UNIT|EQNS|BITS)\.(.*)$", body)

    if match:
        logger.debug("Attempting to parse telemetry-message packet")
        form, body = match[0]

        pd = {"format": "telemetry-message"}
        parsed.update(pd)

        if form in ["PARM", "UNIT"]:
            vals = body.rstrip().split(",")[:13]

            for val in vals:
                if not re.match(r"^(.{1,20}|)$", val):
                    raise Exception("incorrect format of %s (name too long?)" % form)

            defvals = [""] * 13
            defvals[:len(vals)] = vals

            pd = {"t%s" % form: defvals}
            parsed.update(pd)

        elif form == "EQNS":
            eqns = body.rstrip().split(",")[:15]
            teqns = [0, 1, 0] * 5

            for idx, val in enumerate(eqns):
                if not re.match(r"^([-]?\d*\.?\d+|)$", val):
                    raise Exception("value at %d is not a number in %s" % (idx + 1, form))
                else:
                    try:
                        val = int(val)
                    except:
                        val = float(val) if val != "" else 0

                    teqns[idx] = val

            # group values in 5 list of 3
            teqns = [teqns[i*3:(i+1)*3] for i in range(5)]
            pd = {"t%s" % form: teqns}
            parsed.update(pd)

        elif form == "BITS":
            match = re.findall(r"^([01]{8}),(.{0,23})$", body.rstrip())
            if not match:
                raise Exception("incorrect format of %s (title too long?)" % form)

            bits, title = match[0]

            pd = {"t%s" % form: bits,"title": title.strip(" ")}
            parsed.update(pd)

    rl = (body, parsed)
    return rl

