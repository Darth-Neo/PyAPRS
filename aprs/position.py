...
import re
from base91 import *
from common import parse_timestamp, parse_comment
...
def parse_position(packet_type, body):
    ...
    if packet_type == ";":
        ...
        if match:
            name, flag = match[0]
            pd = {"object_name": name,"alive": flag == "*",}
            parsed.update(pd)
            body = body[10:]
        else:
            raise Exception("invalid format")
    else:
        pd = {"messagecapable": packet_type in "@="}
        parsed.update(pd)
    ...
    if len(body) == 0 and "timestamp" in parsed:
        raise Exception("invalid position report format")
    ...
    if parsed["symbol"] == "_":
        ...
        pd = {"comment": body.strip(" "),"weather": result,}
        parsed.update(pd)
    else:
        # decode comment
    ...
    if packet_type == ";":
        pd = {"object_format": parsed["format"], "format": "object", }
        parsed.update(pd)
...
def parse_compressed(body):
    ...
    if re.match(r"^[\/\\A-Za-j][!-|]{8}[!-{}][ -|]{3}", body):
        ...
        if len(body) < 13:
            raise Exception("Invalid compressed packet (less than 13 characters)")
        ...
        try:
            latitude = 90 - (to_decimal(compressed[1:5]) / 380926.0)
            longitude = -180 + (to_decimal(compressed[5:9]) / 190463.0)
        except ValueError:
            raise Exception("invalid characters in latitude/longitude encoding")
        ...
        if c1 == -1:
            pd = {"gpsfixstatus": 1 if ctype & 0x20 == 0x20 else 0}
            parsed.update(pd)
        ...
        if -1 in [c1, s1]:
            ...
            pd = {"course": 360 if c1 == 0 else c1 * 4}
            parsed.update(pd)

            pd = {"speed": (1.08 ** s1 - 1) * 1.852} # mul = convert knts to kmh
            parsed.update(pd)

        elif c1 == 90:
            pd = {"radiorange": (2 * 1.08 ** s1) * 1.609344} # mul = convert mph to kmh
            parsed.update(pd)

        pd = {"symbol": symbol,"symbol_table": symbol_table,"latitude": latitude,"longitude": longitude,}
        parsed.update(pd)
...
def parse_normal(body):
    ...
    if match:
        ...
        if posambiguity != lon_min.count(" "):
            raise Exception("latitude and longitude ambiguity mismatch")

        pd = {"posambiguity": posambiguity}
        parsed.update(pd)
        ...
        if int(lat_deg) > 89 or int(lat_deg) < 0:
            raise Exception("latitude is out of range (0-90 degrees)")
        if int(lon_deg) > 179 or int(lon_deg) < 0:
            raise Exception("longitutde is out of range (0-180 degrees)")
        """
        f float(lat_min) >= 60:
